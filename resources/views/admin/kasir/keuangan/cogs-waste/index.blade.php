...
@section('title', 'Bahan Terbuang (Waste Log)')
...
<div class="page-header">
  <div>
    <h1>Pencatatan Bahan Terbuang (Waste Log)</h1>
  ...
  <a href="{{ route('admin.keuangan.cogs-waste.create') }}" class="btn btn-primary-grad">
    <i class="bi bi-plus-lg me-1"></i>Catat Bahan Terbuang
  </a>
</div>

<div class="card">
  <div class="card-header-flex">
    <h6>Daftar Bahan Terbuang / Basi (Waste Log)</h6>
    <div class="d-flex align-items-center gap-2">
      ...
      <span class="chip-tag" id="totalCount">{{ $wasteLogs->total() }} item</span>
    </div>
  </div>
  ...
  <div class="px-3 pt-3 pb-2">
    <div class="input-group">
      ...
      <input type="text" id="searchInput" class="form-control form-control-modern border-start-0 ps-0" placeholder="Cari nama bahan atau alasan terbuang...">
    </div>
  </div>
  ...
  <div class="card-body p-0">
    <div class="table-responsive" id="tableContainer">
      <div id="tableBody">
        @include('admin.kasir.keuangan.cogs-waste._data')
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content text-center p-3" style="background: var(--bg-surface); border: 1px solid var(--border-subtle); color: var(--text-primary);">
      ...
      <p class="text-muted-c style-sub text-wrap mb-3">Apakah Anda yakin ingin menghapus catatan waste ini?</p>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadData(page = 1) {
    ...
    fetch(`{{ route('admin.keuangan.cogs-waste.data') }}?page=${page}&per_page=${perPage}&search=${encodeURIComponent(search)}`, {
  ...
  if (btnConfirmDelete) {
    btnConfirmDelete.addEventListener('click', function() {
      ...
      fetch(`{{ url('admin/keuangan/cogs-waste') }}/${deleteId}`, {
        method: 'DELETE',
        headers: {
      ...
      .catch(err => {
        btn.classList.remove('disabled');
        alert('Gagal menghapus catatan waste.');
      });
    });
  }
