...
<div class="page-header">
  ...
  <a href="{{ route('admin.category.create') }}" class="btn btn-primary-grad">
...
<div class="card">
  <div class="card-header-flex">
    ...
    <div class="d-flex align-items-center gap-2">
      ...
      <span class="chip-tag" id="totalCount">{{ $categories->total() }} item</span>
    </div>
  </div>
  <div class="card-body p-0">
    <div class="table-responsive" id="tableContainer">
      <table class="table-modern" id="dataTable">
        ...
        <tbody id="tableBody">
          @include('admin.kasir.category._data', ['categories' => $categories])
        </tbody>
      </table>
    </div>
    <div class="px-3 py-2 d-flex justify-content-between align-items-center" id="paginationContainer">
      <span class="text-muted-c" style="font-size:0.85rem;" id="pageInfo">
        Menampilkan {{ $categories->firstItem() ?? 0 }} - {{ $categories->lastItem() ?? 0 }} dari {{ $categories->total() }}
      </span>
      {{ $categories->onEachSide(1)->links('vendor.pagination.modern') }}
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-sm modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body text-center py-4">
        ...
        <p class="mt-2 mb-0">Yakin ingin menghapus kategori ini?</p>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadData(page, perPageVal) {
    ...
    fetch(`{{ route('admin.category.data') }}?page=${page}&per_page=${perPageVal}`, {
  ...
  function attachHandlers() {
    ...
    document.querySelectorAll('.btn-delete').forEach(function(btn) {
      btn.addEventListener('click', function() {
        // Set data-url ke modal, lalu buka modal
