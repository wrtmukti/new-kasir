...
<div class="card">
  <div class="card-header-flex">
    ...
    <div class="d-flex align-items-center gap-2">
      ...
      <span class="chip-tag" id="totalCount">{{ $stocks->total() }} item</span>
    </div>
  </div>
  <div class="card-body p-0">
    <div class="table-responsive" id="tableContainer">
      <table class="table-modern" id="dataTable">
        ...
        <tbody id="tableBody">
          @include('admin.kasir.stock._data', ['stocks' => $stocks])
        </tbody>
      </table>
    </div>
    <div class="px-3 py-2 d-flex justify-content-between align-items-center" id="paginationContainer">
      <span class="text-muted-c" style="font-size:0.85rem;" id="pageInfo">
        Menampilkan {{ $stocks->firstItem() ?? 0 }} - {{ $stocks->lastItem() ?? 0 }} dari {{ $stocks->total() }}
      </span>
      {{ $stocks->onEachSide(1)->links('vendor.pagination.modern') }}
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadData(page, perPageVal) {
    ...
    fetch(`{{ route('admin.stock.data') }}?page=${page}&per_page=${perPageVal}`, {
