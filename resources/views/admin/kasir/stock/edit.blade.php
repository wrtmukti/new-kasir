...
<form action="{{ route('admin.stock.update', $stock) }}" method="POST" class="form-submit-loading">
    ...
    <div class="card-body">
      <div class="row g-3">
          ...
          <div class="input-skeleton">
            <select name="outlet_id" class="form-select-modern">
              ...
              @foreach($outlets as $c)
                <option value="{{ $c->outlet_id }}" {{ old('outlet_id', $stock->outlet_id) == $c->outlet_id ? 'selected' : '' }}>{{ $c->outlet_name }}</option>
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="col-md-4">
          ...
          <div class="input-skeleton">
            <input type="text" name="stock_name" id="stock_name" class="form-control-modern @error('stock_name') is-invalid @enderror" value="{{ old('stock_name', $stock->stock_name) }}" placeholder="Nama bahan baku">
        ...
        <div class="col-md-3">
          ...
          <div class="input-skeleton">
            <input type="number" name="stock_amount" id="stock_amount" class="form-control-modern" value="{{ old('stock_amount', $stock->stock_amount) }}" min="0">
          </div>
        </div>
        <div class="col-md-3">
  ...
  <div class="d-flex gap-2">
    <button type="submit" class="btn btn-primary-grad btn-loading" id="btnSubmit">Simpan Perubahan</button>
