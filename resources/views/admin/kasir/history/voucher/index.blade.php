  ...
  <div class="card-body p-0">
    <div class="table-responsive" id="tableContainer">
      <table class="table-modern" id="dataTable">
        ...
        <tbody id="tableBody">
          @include('admin.kasir.history.voucher._data', ['histories' => $histories])
        </tbody>
      </table>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadData(page, perPageVal) {
    const url = '{{ route("admin.history.voucher.data") }}?page=' + page + '&per_page=' + perPageVal;

    // Skeleton
    tableBody.innerHTML = '';
    for (let i = 0; i < Math.min(perPageVal, 5); i++) {
      const tr = document.createElement('tr');
      tr.innerHTML = '<td colspan="8"><div class="skeleton skeleton-text" style="height:20px;margin:4px 0;"></div></td>';
