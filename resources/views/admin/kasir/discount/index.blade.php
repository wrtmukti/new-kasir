...
<div class="page-header">
  ...
  <a href="{{ route('admin.discount.create') }}" class="btn btn-primary-grad">
...
<div class="card">
  <div class="card-header-flex">
    ...
    <div class="d-flex align-items-center gap-2">
      ...
      <span class="chip-tag" id="totalCount">{{ $discounts->total() }} item</span>
    </div>
  </div>
  <div class="card-body p-0">
    <div class="table-responsive" id="tableContainer">
      <table class="table-modern" id="dataTable">
        ...
        <tbody id="tableBody">
          @include('admin.kasir.discount._data', ['discounts' => $discounts])
        </tbody>
      </table>
    </div>
    <div class="px-3 py-2 d-flex justify-content-between align-items-center" id="paginationContainer">
      <span class="text-muted-c" style="font-size:0.85rem;" id="pageInfo">
        Menampilkan {{ $discounts->firstItem() ?? 0 }} - {{ $discounts->lastItem() ?? 0 }} dari {{ $discounts->total() }}
      </span>
      {{ $discounts->onEachSide(1)->links('vendor.pagination.modern') }}
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-sm modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body text-center py-4">
        ...
        <p class="mt-2 mb-0">Yakin ingin menghapus diskon ini?</p>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadData(page, perPageVal) {
    ...
    fetch(`{{ route('admin.discount.data') }}?page=${page}&per_page=${perPageVal}`, {
