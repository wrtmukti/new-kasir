...
<div class="card">
  ...
  <div class="card-body">
    <form action="{{ route('admin.table.store') }}" method="POST" class="form-submit-loading">
      @csrf
      <div class="row g-3">
        <div class="col-md-4">
        ...
        <div class="col-md-4">
          ...
          <div class="input-skeleton">
            <input type="number" name="table_number" class="form-control-modern @error('table_number') is-invalid @enderror" value="{{ old('table_number') }}" placeholder="1" min="1">
            @error('table_number')
        ...
        <div class="col-md-4">
          ...
          <div class="input-skeleton">
            <input type="number" name="table_capacity" class="form-control-modern" value="{{ old('table_capacity', 4) }}" min="1" placeholder="4">
          </div>
        </div>
        <div class="col-md-6">
          <label class="form-label-modern">Deskripsi</label>
          <div class="input-skeleton">
            <textarea name="table_description" class="form-control-modern" rows="3" placeholder="Deskripsi meja">{{ old('table_description') }}</textarea>
          </div>
        </div>
        <div class="col-md-3">
          ...
          <div class="input-skeleton">
            <select name="table_status" class="form-select-modern">
              <option value="active" {{ old('table_status', 'active') == 'active' ? 'selected' : '' }}>Kosong</option>
              <option value="reserved" {{ old('table_status') == 'reserved' ? 'selected' : '' }}>Dipesan</option>
              <option value="occupied" {{ old('table_status') == 'occupied' ? 'selected' : '' }}>Terisi</option>
              <option value="inactive" {{ old('table_status') == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
            </select>
          </div>
        </div>
