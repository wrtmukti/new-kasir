...
<div class="card">
  ...
  <div class="card-body">
    <form action="{{ route('admin.product.store') }}" method="POST" class="form-submit-loading" enctype="multipart/form-data" id="productForm">
      @csrf

      {{-- Hidden field to track selected stock IDs --}}
            ...
            <div class="input-skeleton">
              <select name="discount_id" class="form-select-modern @error('discount_id') is-invalid @enderror">
                ...
                @foreach($discounts as $d)
                  <option value="{{ $d->discount_id }}" {{ old('discount_id') == $d->discount_id ? 'selected' : '' }}>
                    {{ $d->discount_name }}
      ...
      <div class="step-panel" id="step2Panel" style="display:none;">
          ...
          <div class="input-skeleton">
            ...
            <div class="stock-tag-container" id="stockTagContainer">
              @foreach($stocks as $stock)
                <span class="stock-tag tag" data-stock-id="{{ $stock->stock_id }}"
            ...
            @if($stocks->isEmpty())
              <div class="text-center text-muted-c py-4">
                ...
                Belum ada data stok. <a href="{{ route('admin.stock.create') }}" style="color:var(--accent-1);">Tambah stok</a> dulu.
              </div>
            @endif
          </div>
        </div>
        ...
        <div class="selected-stock-summary" id="selectedSummary">
          <label class="form-label-modern">Stok Terpilih <span class="selected-count" id="selectedCount">0</span></label>
...
.stock-tag-container::-webkit-scrollbar {
  width: 4px;
}
.stock-tag-container::-webkit-scrollbar-thumb {
  background: var(--border-subtle);
  border-radius: 4px;
}

/* ── Stock Tag (unselected) ── */
.stock-tag {
  cursor: pointer;
...
.stock-tag .tag-add-icon {
  opacity: 0.4;
  transition: opacity 0.2s;
}
.stock-tag:hover .tag-add-icon {
  opacity: 1;
...
.stock-tag.selected .tag-add-icon::before {
  content: "\F633"; /* bi-check-circle-fill */
}
.stock-tag.selected .tag-add-icon {
  opacity: 1;
  color: var(--accent-1) !important;
}
...
.qty-row-input .form-control-modern {
  width: 100px;
  text-align: center;
  font-weight: 600;
}
.qty-row-input .unit-label {
  font-size: 0.78rem;
  color: var(--text-muted);
  font-weight: 500;
  min-width: 30px;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let currentStep = 1;
  let selectedStocks = []; // array of { id, name, code, unit, price }
  ...
  const emptyQtyMsg      = document.getElementById('emptyQtyMsg');
  const stockIdsInput    = document.getElementById('stockIdsInput');
  const hasStocksInput   = document.getElementById('hasStocksInput');
  ...
  // ─── Update Selected Tags & Quantity List ───
  function updateSelectedUI() {
    // Update hidden input
    stockIdsInput.value = JSON.stringify(selectedStocks.map(s => s.id));
    hasStocksInput.value = selectedStocks.length > 0 ? '1' : '0';
    ...
    if (selectedStocks.length === 0) {
      selectedTagsCont.innerHTML = '<span class="text-muted-c" style="font-size:0.85rem;" id="emptySelectedMsg">Belum ada stok dipilih</span>';
    } else {
      let html = '';
      ...
      selectedTagsCont.querySelectorAll('.remove-tag').forEach(function(el) {
        el.addEventListener('click', function() {
          ...
          // Also remove 'selected' class from tag
          document.querySelectorAll('.stock-tag').forEach(function(t) {
            if (t.dataset.stockId === id) t.classList.remove('selected');
          });
          updateSelectedUI();
        });
      });
    ...
    // Regenerate quantity list for step 3
    renderQuantityList();
  }
  ...
  function renderQuantityList() {
    if (selectedStocks.length === 0) {
      quantityList.innerHTML = '<div class="text-center text-muted-c py-4" id="emptyQtyMsg">'
                             + '<i class="bi bi-arrow-left" style="font-size:1.2rem;display:block;margin-bottom:0.5rem;"></i>'
                             + 'Pilih stok di langkah sebelumnya</div>';
      return;
    }

    let html = '';
    selectedStocks.forEach(function(s, index) {
      const qtyVal = document.querySelector('input[name="quantities[' + index + ']"]');
      const val = qtyVal ? qtyVal.value : '1';
      html += '<div class="qty-row" data-index="' + index + '">'
            ...
            +       (s.price > 0 ? '<span><i class="bi bi-coin" style="font-size:0.65rem;"></i>Rp ' + Number(s.price).toLocaleString('id-ID') + '</span>' : '')
            +     '</div>'
            +   '</div>'
            +   '<div class="qty-row-input">'
            +     '<input type="number" name="quantities[' + index + ']" class="form-control-modern" value="' + val + '" min="0" step="1" placeholder="0">'
  ...
  function goToStep(step) {
    ...
    // If entering step 3, ensure quantity list is fresh
    if (step === STEP_AMOUNT) {
      renderQuantityList();
    }
  ...
  function validateStep(step) {
    if (step === STEP_PRODUCT) {
      ...
      if (!name.value.trim()) {
        ...
        setTimeout(function() {
          name.closest('.input-skeleton').classList.remove('shake');
        }, 350);
        name.focus();
    ...
    if (step === STEP_STOCKS) {
      // Allow proceeding even if no stocks selected (product can exist without ingredient mapping)
      return true;
    }
  ...
  nextBtn.addEventListener('click', function() {
    ...
    if (currentStep === STEP_PRODUCT) {
      goToStep(STEP_STOCKS);
    } else if (currentStep === STEP_STOCKS) {
      goToStep(STEP_AMOUNT);
    }
  });
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      // Validate quantity inputs not empty
      const qtyInputs = form.querySelectorAll('input[name^="quantities["]');
      let allZero = true;
      qtyInputs.forEach(function(inp) {
        if (parseInt(inp.value) > 0) allZero = false;
      });

      // Sync stock_ids before submit
      stockIdsInput.value = JSON.stringify(selectedStocks.map(s => s.id));
  ...
  // ─── Init: if old values exist, re-select stocks ───
  (function initFromOld() {
    try {
      const oldIds = JSON.parse(stockIdsInput.value || '[]');
      if (Array.isArray(oldIds) && oldIds.length > 0) {
        stockTags.forEach(function(tag) {
          if (oldIds.includes(tag.dataset.stockId)) {
            const id    = tag.dataset.stockId;
            const name  = tag.dataset.stockName;
            const code  = tag.dataset.stockCode;
            const unit  = tag.dataset.stockUnit;
            const price = tag.dataset.stockPrice;
            if (!selectedStocks.find(s => s.id === id)) {
              selectedStocks.push({ id, name, code, unit, price });
