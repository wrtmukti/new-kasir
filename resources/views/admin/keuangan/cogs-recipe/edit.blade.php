...
@section('title', 'Edit Resep Standar HPP')
...
<div class="page-header">
  <div>
    <h1>Edit Resep Standar & COGS Menu</h1>
...
<form action="{{ route('admin.keuangan.cogs-recipe.update', $cogsRecipe) }}" method="POST" id="recipeForm">
  @csrf
  @method('PUT')
      ...
      <div class="card">
        ...
        <div class="card-body p-4">
          <div class="mb-3">
            ...
            <input type="text" name="recipe_name" id="recipe_name" class="form-control-modern @error('recipe_name') is-invalid @enderror" value="{{ old('recipe_name', $cogsRecipe->recipe_name) }}" required>
            @error('recipe_name')
          ...
          <div class="mb-3">
            ...
            <select name="product_id" id="product_id" class="form-select-modern">
              ...
              @foreach($products as $prod)
                <option value="{{ $prod->product_id }}" {{ old('product_id', $cogsRecipe->product_id) == $prod->product_id ? 'selected' : '' }}>
          ...
          <div class="mb-3">
            ...
            <input type="number" step="0.1" name="target_food_cost" id="target_food_cost" class="form-control-modern @error('target_food_cost') is-invalid @enderror" value="{{ old('target_food_cost', $cogsRecipe->target_food_cost) }}" required>
          ...
          <div class="mb-3">
            ...
            <textarea name="notes" id="notes" rows="3" class="form-control-modern">{{ old('notes', $cogsRecipe->notes) }}</textarea>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="card-body p-4">
          <div class="table-responsive mb-3">
            <table class="table-modern" id="recipeItemsTable">
              ...
              <tbody>
                @foreach($cogsRecipe->items as $idx => $item)
                <tr class="item-row">
                  <td>
                    <select name="items[{{ $idx }}][cogs_raw_material_id]" class="form-select-modern select-material" required>
                      <option value="">-- Pilih Bahan Mentah --</option>
                      @foreach($rawMaterials as $raw)
                        <option value="{{ $raw->cogs_raw_material_id }}" data-price="{{ $raw->effective_price }}" data-unit="{{ $raw->unit }}" {{ $item->cogs_raw_material_id == $raw->cogs_raw_material_id ? 'selected' : '' }}>
          ...
          <div class="d-flex justify-content-end gap-2">
            ...
            <button type="submit" class="btn btn-primary-grad px-4 btn-loading" id="btnSubmit">Perbarui Resep Standar</button>
          </div>
        </div>
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  let rowIdx = {{ $cogsRecipe->items->count() }};
