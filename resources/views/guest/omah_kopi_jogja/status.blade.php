  ...
  <div class="flex items-center justify-between mb-8">
    <div class="flex items-center gap-4">
      <div class="bg-primary p-3.5 rounded-2xl text-white shadow-md">
        <span class="material-symbols-outlined text-3xl fill-icon">coffee</span>
      </div>
      <div>
        <h2 class="font-headline font-black text-2xl md:text-3xl text-primary leading-tight">Omah Tracker</h2>
        <p class="text-xs md:text-sm text-on-surface-variant font-bold">Status Pesanan Meja {{ $table->table_number }}</p>
      </div>
    </div>

    <button type="button" id="okRefreshBtn" class="bg-surface-container-high hover:bg-surface-variant text-on-surface p-2.5 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-xs" title="Refresh Status">
          ...
          @if($status !== 'cancelled')
            <div class="my-6 px-2">
              <div class="grid grid-cols-3 gap-2 relative text-center">
                ...
                <!-- Step 2: Dimasak -->
                <div class="flex flex-col items-center gap-1.5 z-10">
                  <div class="w-10 h-10 rounded-full {{ $stepActive ? 'bg-primary text-white' : 'bg-surface-container-high text-on-surface-variant' }} flex items-center justify-center font-bold shadow-xs border-2 border-white">
...
document.addEventListener('DOMContentLoaded', function() {
  const refreshBtn = document.getElementById('okRefreshBtn');
  if (refreshBtn) {
