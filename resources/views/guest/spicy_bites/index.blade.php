@extends('guest.spicy_bites.layouts.app')
...
<section class="mb-8">
  <h2 class="font-headline font-black text-4xl md:text-5xl text-on-background leading-tight">
    Fuel Your <br/><span class="text-primary">Fire.</span>
  </h2>
  <p class="text-on-surface-variant text-sm md:text-base max-w-md mt-2">
    Cita rasa berani & bumbu istimewa. Pilihlah menu favoritmu di meja {{ $table->table_number }}.
  </p>
</section>
...
<section class="mb-6">
  <div class="relative w-full max-w-md">
    ...
    <input type="text" id="guestSearch" placeholder="Cari menu nikmat..." 
...
<section class="mb-8 -mx-4 px-4 overflow-hidden">
  <div class="flex gap-2.5 overflow-x-auto hide-scrollbar pb-2" id="guestCategoryContainer">
    <button class="sb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full bg-primary text-white font-headline font-bold text-xs transition-all active:scale-95 active" data-cat="all">
      Semua Menu
    </button>
    @if($bundles->isNotEmpty())
      <button class="sb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full border-2 border-outline-variant text-on-surface-variant font-headline font-bold text-xs hover:bg-surface-container transition-all active:scale-95" data-cat="bundle">
        🔥 Paket Bundle
      </button>
    @endif
    @foreach($categories as $cat)
      <button class="sb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full border-2 border-outline-variant text-on-surface-variant font-headline font-bold text-xs hover:bg-surface-container transition-all active:scale-95" data-cat="{{ $cat->category_id }}">
        {{ $cat->category_name }}
      </button>
    @endforeach
  </div>
  ...
  <div class="mb-8" id="guestBundlesSection">
    ...
    <div class="bento-grid" id="guestBundles">
      @foreach($bundles as $bundle)
        @include('guest.spicy_bites.partials._bundle_card', ['bundle' => $bundle])
      @endforeach
    </div>
  </div>
@endif
...
<section>
  <div class="flex items-center justify-between mb-4">
    ...
    <button type="button" id="sbViewToggle" class="p-2 rounded-full bg-surface-container-low hover:bg-surface-variant text-primary transition-colors flex items-center justify-center active:scale-95 shadow-xs" title="Tampilan Daftar">
      <span class="material-symbols-outlined text-[20px]" id="sbViewToggleIcon">format_list_bulleted</span>
    </button>
  </div>

  <div class="bento-grid" id="guestProducts">
    @foreach($products as $product)
      @include('guest.spicy_bites.partials._product_card', ['product' => $product])
    @endforeach
  </div>
...
<div id="sbCartBar" class="sb-cart-bar hidden cursor-pointer">
  <div class="flex items-center gap-3">
    <div class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center">
      <span class="material-symbols-outlined text-[20px] fill-icon">shopping_bag</span>
    </div>
    <div>
      <div id="sbCartCount" class="font-headline font-bold text-xs uppercase tracking-wide opacity-90">0 Item</div>
      <div id="sbCartTotal" class="font-headline font-black text-sm">Rp 0</div>
    </div>
  </div>
...
<!-- Cart Sheet Backdrop & Bottom Sheet -->
<div id="sbCartBackdrop" class="sb-cart-backdrop"></div>
<div id="sbCartSheet" class="sb-cart-sheet">
  ...
  <div class="px-6 py-3 border-b border-surface-variant flex items-center justify-between">
    ...
    <button type="button" id="sbCartClose" class="text-on-surface-variant hover:text-primary p-1 rounded-full">
      <span class="material-symbols-outlined">close</span>
    </button>
  </div>

  <div id="sbCartBody" class="p-6 overflow-y-auto max-h-[50vh] space-y-4 divide-y divide-surface-variant">
    <!-- Rendered via JS -->
  </div>
  ...
  <div class="p-6 border-t border-surface-variant bg-surface-container-low rounded-b-1.5rem">
    <div class="flex items-center justify-between mb-4">
      ...
      <span id="sbSheetTotal" class="font-headline font-black text-xl text-primary">Rp 0</span>
    </div>
    
    <form id="sbCheckoutForm" method="POST" action="{{ route('guest.checkout') }}">
      @csrf
      <input type="hidden" name="table_id" value="{{ $table->table_id }}"/>
      <input type="hidden" name="total_price" id="sbFormTotalPrice" value="0"/>
      <input type="hidden" name="cart_data" id="sbFormCartData" value="[]"/>
      <input type="hidden" name="bundle_data" id="sbFormBundleData" value="[]"/>
      
      <button type="submit" id="sbCheckoutBtn" class="w-full bg-primary hover:bg-primary-container text-white py-3.5 rounded-xl font-headline font-extrabold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
...
<div id="sbItemModal" class="sb-modal-overlay">
  <div class="sb-modal-card">
    <div class="flex items-center justify-between border-b border-surface-variant pb-3 mb-4">
      <h4 id="sbModalTitle" class="font-headline font-extrabold text-base text-on-background truncate">Nama Menu</h4>
      <button type="button" id="sbModalClose" class="text-on-surface-variant hover:text-primary">
        <span class="material-symbols-outlined">close</span>
      </button>
    </div>
    ...
    <div class="mb-4">
      <div class="flex items-center justify-between mb-3">
        <span id="sbModalPrice" class="font-headline font-black text-lg text-primary">Rp 0</span>
        <div class="flex items-center gap-3 bg-surface-container-high rounded-full px-3 py-1">
          <button type="button" id="sbQtyMinus" class="w-8 h-8 rounded-full bg-white text-on-background flex items-center justify-center font-bold shadow-xs hover:bg-surface transition-all active:scale-90">
            <span class="material-symbols-outlined text-[18px]">remove</span>
          </button>
          <span id="sbQtyVal" class="font-headline font-bold text-sm min-w-[24px] text-center">1</span>
          <button type="button" id="sbQtyPlus" class="w-8 h-8 rounded-full bg-white text-on-background flex items-center justify-center font-bold shadow-xs hover:bg-surface transition-all active:scale-90">
      ...
      <input type="text" id="sbModalNote" placeholder="Cth: tidak pedas, ekstra saus..."
    ...
    <button type="button" id="sbAddToCartBtn" class="w-full bg-primary hover:bg-primary-container text-white py-3 rounded-xl font-headline font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
      <span class="material-symbols-outlined text-[20px]">add_shopping_cart</span>
      <span id="sbAddToCartBtnText">Simpan Pesanan</span>
    </button>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const savedCart = sessionStorage.getItem('sb_guest_cart_{{ $table->table_id }}');
  if (savedCart) {
  ...
  // DOM Elements
  const cartBar = document.getElementById('sbCartBar');
  const cartCountEl = document.getElementById('sbCartCount');
  const cartTotalEl = document.getElementById('sbCartTotal');

  const cartSheet = document.getElementById('sbCartSheet');
  const cartBackdrop = document.getElementById('sbCartBackdrop');
  const cartClose = document.getElementById('sbCartClose');
  const cartBody = document.getElementById('sbCartBody');
  const sheetTotalEl = document.getElementById('sbSheetTotal');

  const modal = document.getElementById('sbItemModal');
  const modalTitle = document.getElementById('sbModalTitle');
  const modalPrice = document.getElementById('sbModalPrice');
  const modalNote = document.getElementById('sbModalNote');
  const modalClose = document.getElementById('sbModalClose');
  const qtyVal = document.getElementById('sbQtyVal');
  const qtyMinus = document.getElementById('sbQtyMinus');
  const qtyPlus = document.getElementById('sbQtyPlus');
  const addToCartBtn = document.getElementById('sbAddToCartBtn');
  const addToCartBtnText = document.getElementById('sbAddToCartBtnText');

  const checkoutForm = document.getElementById('sbCheckoutForm');
  ...
  const viewToggleBtn = document.getElementById('sbViewToggle');
  const viewToggleIcon = document.getElementById('sbViewToggleIcon');
  ...
  function setProductsView(isList) {
    ...
    if (isList) {
      productsGrid.classList.add('sb-list-view');
      viewToggleBtn.title = 'Tampilan Kartu';
      viewToggleIcon.textContent = 'grid_view';
    } else {
      productsGrid.classList.remove('sb-list-view');
      viewToggleBtn.title = 'Tampilan Daftar';
      viewToggleIcon.textContent = 'format_list_bulleted';
    }
  }

  const savedViewMode = sessionStorage.getItem('sb_guest_view') === 'list';
  setProductsView(savedViewMode);

  if (viewToggleBtn) {
    viewToggleBtn.addEventListener('click', function() {
      const isCurrentlyList = productsGrid.classList.contains('sb-list-view');
      const nextIsList = !isCurrentlyList;
      setProductsView(nextIsList);
      sessionStorage.setItem('sb_guest_view', nextIsList ? 'list' : 'grid');
    });
  }

  // Save cart state
  function saveCart() {
    sessionStorage.setItem('sb_guest_cart_{{ $table->table_id }}', JSON.stringify(cart));
    renderCartSummary();
  }
  ...
  // Category Filtering
  const catPills = document.querySelectorAll('.sb-cat-pill');
  ...
  catPills.forEach(pill => {
    pill.addEventListener('click', function() {
      ...
      catPills.forEach(p => p.classList.add('border-2', 'border-outline-variant', 'text-on-surface-variant'));

      this.classList.remove('border-2', 'border-outline-variant', 'text-on-surface-variant');
  ...
  checkoutForm.addEventListener('submit', function(e) {
    ...
    document.getElementById('sbFormTotalPrice').value = grandTotal;
    document.getElementById('sbFormCartData').value = JSON.stringify(itemsPayload);
    document.getElementById('sbFormBundleData').value = JSON.stringify(bundlesPayload);

    // Disable button safely without breaking submit payload
    const checkoutBtn = document.getElementById('sbCheckoutBtn');
