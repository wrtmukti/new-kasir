...
<head>
  ...
  <title>@yield('title', 'Spicy Bites Menu') — {{ $outlet->outlet_name ?? 'Spicy Bites' }}</title>
      ...
      <div class="flex items-center gap-3">
        <a href="{{ route('guest.index', $table->table_id) }}" class="flex items-center gap-2 group">
          @if(!empty($outlet->outlet_image))
            <img src="{{ asset('storage/' . $outlet->outlet_image) }}" alt="Logo" class="w-9 h-9 rounded-full object-cover border border-primary/30 group-hover:scale-105 transition-transform"/>
          @else
          ...
          <h1 class="text-xl font-headline font-black text-primary uppercase tracking-tighter">
            {{ $outlet->outlet_name ?? 'Spicy Bites' }}
          </h1>
        </a>
      </div>
      <div class="flex items-center gap-3">
