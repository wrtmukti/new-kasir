...
<div class="max-w-4xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-8">
    <div class="flex items-center gap-4">
      <div class="bg-primary-container p-3.5 rounded-2xl pulse-flame shadow-md">
        <span class="material-symbols-outlined text-on-primary-container text-3xl fill-icon">local_fire_department</span>
      </div>
      <div>
        <h2 class="font-headline font-black text-2xl md:text-3xl text-primary leading-tight">Spicing it up!</h2>
        <p class="text-xs md:text-sm text-on-surface-variant font-bold">Status Pesanan Meja {{ $table->table_number }}</p>
      </div>
    </div>

    <button type="button" id="sbRefreshBtn" class="bg-surface-container-high hover:bg-surface-variant text-on-surface p-2.5 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-xs" title="Refresh Status">
      <span class="material-symbols-outlined text-[20px]">refresh</span>
    </button>
  </div>

  @if($orders->isEmpty())
    <div class="text-center py-16 bg-white rounded-2xl border border-dashed border-surface-variant shadow-sm my-6">
      ...
      <a href="{{ route('guest.index', $table->table_id) }}" class="inline-flex items-center gap-2 bg-primary hover:bg-primary-container text-white px-6 py-3 rounded-xl font-headline font-bold text-xs shadow-md transition-all active:scale-95">
  ...
  @else
    <div class="space-y-6">
      @foreach($orders as $order)
        ...
        <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-sm bento-card">
          <!-- Order Card Header -->
          ...
          @if($status !== 'cancelled')
            <div class="my-6 px-2">
              <div class="grid grid-cols-3 gap-2 relative text-center">
                ...
                <!-- Step 1: Diterima / Bayar Kasir -->
                <div class="flex flex-col items-center gap-1.5 z-10">
                  <div class="w-10 h-10 rounded-full {{ ($isPre && !$isPaid) ? 'bg-amber-500 text-white pulse-flame' : 'bg-secondary-container text-on-secondary-container' }} flex items-center justify-center font-bold shadow-sm border-2 border-white">
                    <span class="material-symbols-outlined text-[20px] fill-icon">{{ ($isPre && !$isPaid) ? 'point_of_sale' : 'check_circle' }}</span>
                  </div>
                ...
                <!-- Step 2: Dimasak -->
                <div class="flex flex-col items-center gap-1.5 z-10">
                  <div class="w-10 h-10 rounded-full {{ $stepActive ? 'bg-primary text-white pulse-flame' : 'bg-surface-container-high text-on-surface-variant' }} flex items-center justify-center font-bold shadow-sm border-2 border-white">
    ...
    <div class="text-center mt-8">
      <a href="{{ route('guest.index', $table->table_id) }}" class="inline-flex items-center gap-2 bg-secondary-container hover:bg-secondary-fixed text-on-secondary-container px-6 py-3 rounded-full font-headline font-extrabold text-xs shadow-sm transition-all active:scale-95">
