...
<div class="max-w-3xl mx-auto">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      ...
      <p class="text-xs text-on-surface-variant font-bold">Meja {{ $table->table_number }} • {{ $company->company_name ?? 'Spicy Bites' }}</p>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-sm mb-6">
    <h3 class="font-headline font-extrabold text-base text-on-background mb-4 flex items-center gap-2">
    ...
    <div class="divide-y divide-surface-variant">
      ...
      @foreach($bundleRows ?? [] as $b)
        <div class="py-3.5 flex items-center justify-between gap-3">
          <div class="flex items-center gap-3 flex-grow">
            <div class="w-14 h-14 rounded-xl bg-secondary-container/30 flex items-center justify-center text-on-secondary-container flex-shrink-0 border border-secondary-container">
              <span class="material-symbols-outlined text-2xl fill-icon">local_offer</span>
            </div>
            <div>
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-sm mb-6">
    ...
    <div class="flex gap-2">
      <input type="text" id="sbVoucherInput" value="{{ old('voucher_code') }}" placeholder="Masukkan kode voucher (cth: SPICY10)"
      ...
      <button type="button" id="sbVoucherBtn" class="bg-secondary-container hover:bg-secondary-fixed text-on-secondary-container px-5 py-2.5 rounded-xl font-headline font-extrabold text-xs transition-all active:scale-95 shadow-xs">
        Gunakan
      </button>
    </div>
    <div id="sbVoucherResult" class="mt-2 text-xs"></div>
  </div>
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-sm mb-6">
    ...
    <textarea id="sbOrderRemark" rows="2" placeholder="Cth: tolong dipisahkan sambalnya, disajikan bersamaan..."
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-sm mb-6 space-y-3">
    ...
    <div id="sbVoucherRow" class="flex justify-between items-center text-sm font-headline hidden">
      <span class="text-primary font-medium flex items-center gap-1">
        <span class="material-symbols-outlined text-[16px]">sell</span>
        Diskon Voucher
      </span>
      <span id="sbVoucherAmount" class="font-bold text-primary">-Rp 0</span>
    </div>
    ...
    <div class="border-t border-surface-variant pt-3 flex justify-between items-center text-lg font-headline">
      ...
      <span id="sbGrandTotal" class="font-black text-primary text-xl">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
    </div>
  </div>

  <!-- Submit Form -->
  <form id="sbSubmitForm" method="POST" action="{{ route('guest.submit') }}">
    @csrf
    <input type="hidden" name="table_id" value="{{ $table->table_id }}"/>
    <input type="hidden" name="total_price" id="sbFormTotalPrice" value="{{ $grandTotal }}"/>
    <input type="hidden" name="items" id="sbFormItems"/>
    <input type="hidden" name="bundles" id="sbFormBundles"/>
    <input type="hidden" name="voucher_code" id="sbFormVoucherCode"/>
    <input type="hidden" name="order_remark" id="sbFormOrderRemark"/>

    <button type="submit" id="sbSubmitBtn" class="w-full bg-primary hover:bg-primary-container text-white py-4 rounded-xl font-headline font-black text-base flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const voucherInput = document.getElementById('sbVoucherInput');
  const voucherBtn = document.getElementById('sbVoucherBtn');
  const voucherResult = document.getElementById('sbVoucherResult');
  const voucherRow = document.getElementById('sbVoucherRow');
  const voucherAmountEl = document.getElementById('sbVoucherAmount');
  const grandTotalEl = document.getElementById('sbGrandTotal');

  const submitForm = document.getElementById('sbSubmitForm');
  ...
  function updateTotals() {
    ...
    document.getElementById('sbFormTotalPrice').value = finalTotal;
  ...
  submitForm.addEventListener('submit', function(e) {
    ...
    document.getElementById('sbFormItems').value = JSON.stringify(items);
    document.getElementById('sbFormBundles').value = JSON.stringify(bundles.map(b => ({
    ...
    document.getElementById('sbFormVoucherCode').value = appliedVoucher ? appliedVoucher.code : '';
    document.getElementById('sbFormOrderRemark').value = document.getElementById('sbOrderRemark').value.trim();

    // Clear saved local cart after submit
    sessionStorage.removeItem('sb_guest_cart_{{ $table->table_id }}');
