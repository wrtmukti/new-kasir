...
<head>
  ...
  <title>@yield('title', 'Midnight Social') — {{ $outlet->outlet_name ?? 'Midnight Social' }}</title>
      ...
      <div class="flex items-center gap-3">
        <a href="{{ route('guest.index', $table->table_id) }}" class="flex items-center gap-2.5 group">
          @if(!empty($outlet->outlet_image))
            <img src="{{ asset('storage/' . $outlet->outlet_image) }}" alt="Logo" class="w-9 h-9 rounded-xl object-cover border border-purple-500/40 group-hover:scale-105 transition-transform"/>
          @else
          ...
          <h1 class="text-xl font-headline font-extrabold text-purple-300 tracking-tight">
            {{ $outlet->outlet_name ?? 'Midnight Social' }}
          </h1>
        </a>
      </div>
      <div class="flex items-center gap-3">
