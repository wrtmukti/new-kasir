...
<title>@yield('title', 'Menu') — {{ $company->company_name ?? 'Kasir POS' }}</title>
  ...
  <div class="guest-navbar-inner">
    <a href="{{ route('guest.index', $table->table_id) }}" class="guest-brand">
      @if(!empty($company->company_image))
        <img src="{{ asset('storage/' . $company->company_image) }}" alt="" class="guest-brand-img">
      @else
        <span class="guest-brand-mark">{{ mb_substr($company->company_name ?? 'K', 0, 1) }}</span>
      @endif
      <span class="guest-brand-name">{{ $company->company_name ?? 'Kasir POS' }}</span>
    </a>
