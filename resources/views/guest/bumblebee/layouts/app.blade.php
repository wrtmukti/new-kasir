...
<head>
  ...
  <title>@yield('title', 'Bumblebee Cafe') — {{ $company->company_name ?? 'Bumblebee Cafe' }}</title>
      ...
      <div class="flex items-center gap-3">
        <a href="{{ route('guest.index', $table->table_id) }}" class="flex items-center gap-2.5 group">
          @if(!empty($company->company_image))
            <img src="{{ asset('storage/' . $company->company_image) }}" alt="Logo" class="w-9 h-9 rounded-xl object-cover border border-amber-300 group-hover:scale-105 transition-transform"/>
          @else
          ...
          <h1 class="text-xl font-headline font-black text-amber-800 tracking-tight">
            {{ $company->company_name ?? 'Bumblebee Cafe' }}
          </h1>
        </a>
      </div>
      <div class="flex items-center gap-3">
