@extends('guest.bumblebee.layouts.app')
...
<section class="mb-8">
  <span class="text-xs font-headline font-extrabold uppercase tracking-widest text-amber-900 bg-amber-100 px-3 py-1 rounded-full border border-amber-300">
    Bright Sweets & Golden Brews
  </span>
  <h2 class="font-headline font-black text-3xl md:text-5xl text-on-background leading-tight mt-2">
    Sweeten Your <br/><span class="text-amber-600">Moments.</span>
  </h2>
  <p class="text-on-surface-variant text-sm md:text-base max-w-md mt-2">
    Minuman manis hangat, dessert lezat, & kopi emas segar. Pilihlah menu di meja {{ $table->table_number }}.
  </p>
</section>
...
<section class="mb-6">
  <div class="relative w-full max-w-md">
    ...
    <input type="text" id="guestSearch" placeholder="Cari kue, kopi emas, kopi susu..." 
           class="w-full bg-surface-container-low border border-surface-variant rounded-full pl-11 pr-4 py-2.5 text-sm focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all"/>
  </div>
</section>
...
<section class="mb-8 -mx-4 px-4 overflow-hidden">
  <div class="flex gap-2.5 overflow-x-auto hide-scrollbar pb-2" id="guestCategoryContainer">
    <button class="bb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full bg-amber-400 text-slate-900 font-headline font-bold text-xs transition-all active:scale-95 active shadow-xs" data-cat="all">
      Semua Menu
    </button>
    @if($bundles->isNotEmpty())
      <button class="bb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full border border-outline-variant text-on-surface-variant font-headline font-bold text-xs hover:bg-surface-container transition-all active:scale-95" data-cat="bundle">
        🍯 Paket Honey
      </button>
    @endif
    @foreach($categories as $cat)
      <button class="bb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full border border-outline-variant text-on-surface-variant font-headline font-bold text-xs hover:bg-surface-container transition-all active:scale-95" data-cat="{{ $cat->category_id }}">
        {{ $cat->category_name }}
      </button>
    @endforeach
  </div>
...
@if($bundles->isNotEmpty())
  <div class="mb-8" id="guestBundlesSection">
    <h3 class="font-headline font-extrabold text-lg text-on-background mb-4 flex items-center gap-2">
      <span class="material-symbols-outlined text-amber-600 fill-icon">emoji_nature</span>
      Paket Hemat Honey
    </h3>
    <div class="bento-grid" id="guestBundles">
      @foreach($bundles as $bundle)
        @include('guest.bumblebee.partials._bundle_card', ['bundle' => $bundle])
      @endforeach
    </div>
  </div>
@endif
...
<section>
  <div class="flex items-center justify-between mb-4">
    <h3 class="font-headline font-extrabold text-lg text-on-background flex items-center gap-2">
      <span class="material-symbols-outlined text-amber-600">bakery_dining</span>
      Daftar Menu
    </h3>

    <!-- View Toggle Button (Grid vs List) -->
    <button type="button" id="bbViewToggle" class="p-2 rounded-full bg-surface-container-low border border-surface-variant hover:bg-surface-variant text-amber-800 transition-colors flex items-center justify-center active:scale-95 shadow-xs" title="Tampilan Daftar">
      <span class="material-symbols-outlined text-[20px]" id="bbViewToggleIcon">format_list_bulleted</span>
    </button>
  </div>

  <div class="bento-grid" id="guestProducts">
    @foreach($products as $product)
      @include('guest.bumblebee.partials._product_card', ['product' => $product])
    @endforeach
  </div>
...
<div id="bbCartBar" class="bb-cart-bar hidden cursor-pointer">
  <div class="flex items-center gap-3">
    <div class="w-9 h-9 rounded-full bg-white/40 text-amber-950 flex items-center justify-center">
      <span class="material-symbols-outlined text-[20px] fill-icon">shopping_bag</span>
    </div>
    <div>
      <div id="bbCartCount" class="font-headline font-bold text-xs uppercase tracking-wide opacity-90">0 Item</div>
      <div id="bbCartTotal" class="font-headline font-black text-sm">Rp 0</div>
    </div>
  </div>
  <button type="button" class="flex items-center gap-1 font-headline font-bold text-xs bg-slate-900 text-amber-300 px-4 py-2 rounded-full shadow-sm hover:scale-105 transition-transform">
    Cek Keranjang <span class="material-symbols-outlined text-[16px]">arrow_forward</span>
  </button>
</div>

<!-- Cart Sheet Backdrop & Bottom Sheet Drawer -->
<div id="bbCartBackdrop" class="bb-cart-backdrop"></div>
<div id="bbCartSheet" class="bb-cart-sheet">
  ...
  <div class="px-6 py-3 border-b border-surface-variant flex items-center justify-between">
    <h4 class="font-headline font-black text-lg text-on-background flex items-center gap-2">
      <span class="material-symbols-outlined text-amber-600 fill-icon">shopping_cart</span>
      Keranjang Saya
    </h4>
    <button type="button" id="bbCartClose" class="text-on-surface-variant hover:text-amber-800 p-1 rounded-full">
      <span class="material-symbols-outlined">close</span>
    </button>
  </div>

  <div id="bbCartBody" class="p-6 overflow-y-auto max-h-[50vh] space-y-4 divide-y divide-surface-variant">
    <!-- Rendered via JS -->
  </div>
  ...
  <div class="p-6 border-t border-surface-variant bg-surface-container-low rounded-b-1.5rem">
    <div class="flex items-center justify-between mb-4">
      ...
      <span id="bbSheetTotal" class="font-headline font-black text-xl text-amber-800">Rp 0</span>
    </div>
    
    <form id="bbCheckoutForm" method="POST" action="{{ route('guest.checkout') }}">
      @csrf
      <input type="hidden" name="table_id" value="{{ $table->table_id }}"/>
      <input type="hidden" name="total_price" id="bbFormTotalPrice" value="0"/>
      <input type="hidden" name="cart_data" id="bbFormCartData" value="[]"/>
      <input type="hidden" name="bundle_data" id="bbFormBundleData" value="[]"/>
      
      <button type="submit" id="bbCheckoutBtn" class="w-full bg-amber-400 hover:bg-amber-500 text-slate-900 py-3.5 rounded-xl font-headline font-extrabold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
...
<div id="bbItemModal" class="bb-modal-overlay">
  <div class="bb-modal-card">
    <div class="flex items-center justify-between border-b border-surface-variant pb-3 mb-4">
      <h4 id="bbModalTitle" class="font-headline font-extrabold text-base text-on-background truncate">Nama Menu</h4>
      <button type="button" id="bbModalClose" class="text-on-surface-variant hover:text-amber-800">
        <span class="material-symbols-outlined">close</span>
      </button>
    </div>
    ...
    <div class="mb-4">
      <div class="flex items-center justify-between mb-3">
        <span id="bbModalPrice" class="font-headline font-black text-lg text-amber-800">Rp 0</span>
        <div class="flex items-center gap-3 bg-surface-container-high rounded-full px-3 py-1">
          <button type="button" id="bbQtyMinus" class="w-8 h-8 rounded-full bg-white text-on-background flex items-center justify-center font-bold shadow-xs hover:bg-surface transition-all active:scale-90">
            <span class="material-symbols-outlined text-[18px]">remove</span>
          </button>
          <span id="bbQtyVal" class="font-headline font-bold text-sm min-w-[24px] text-center">1</span>
          <button type="button" id="bbQtyPlus" class="w-8 h-8 rounded-full bg-white text-on-background flex items-center justify-center font-bold shadow-xs hover:bg-surface transition-all active:scale-90">
      ...
      <input type="text" id="bbModalNote" placeholder="Cth: kurang manis, es sedikit..."
             class="w-full bg-surface-container-low border border-surface-variant rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all"/>
    </div>

    <button type="button" id="bbAddToCartBtn" class="w-full bg-amber-400 hover:bg-amber-500 text-slate-900 py-3 rounded-xl font-headline font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
      <span class="material-symbols-outlined text-[20px]">add_shopping_cart</span>
      <span id="bbAddToCartBtnText">Simpan Pesanan</span>
    </button>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // DOM Elements
  const cartBar = document.getElementById('bbCartBar');
  const cartCountEl = document.getElementById('bbCartCount');
  const cartTotalEl = document.getElementById('bbCartTotal');

  const cartSheet = document.getElementById('bbCartSheet');
  const cartBackdrop = document.getElementById('bbCartBackdrop');
  const cartClose = document.getElementById('bbCartClose');
  const cartBody = document.getElementById('bbCartBody');
  const sheetTotalEl = document.getElementById('bbSheetTotal');

  const modal = document.getElementById('bbItemModal');
  const modalTitle = document.getElementById('bbModalTitle');
  const modalPrice = document.getElementById('bbModalPrice');
  const modalNote = document.getElementById('bbModalNote');
  const modalClose = document.getElementById('bbModalClose');
  const qtyVal = document.getElementById('bbQtyVal');
  const qtyMinus = document.getElementById('bbQtyMinus');
  const qtyPlus = document.getElementById('bbQtyPlus');
  const addToCartBtn = document.getElementById('bbAddToCartBtn');
  const addToCartBtnText = document.getElementById('bbAddToCartBtnText');

  const checkoutForm = document.getElementById('bbCheckoutForm');
  ...
  const viewToggleBtn = document.getElementById('bbViewToggle');
  const viewToggleIcon = document.getElementById('bbViewToggleIcon');
  ...
  function setProductsView(isList) {
    ...
    if (isList) {
      productsGrid.classList.add('bb-list-view');
      viewToggleBtn.title = 'Tampilan Kartu';
      viewToggleIcon.textContent = 'grid_view';
    } else {
      productsGrid.classList.remove('bb-list-view');
      viewToggleBtn.title = 'Tampilan Daftar';
      viewToggleIcon.textContent = 'format_list_bulleted';
    }
  }

  const savedViewMode = sessionStorage.getItem('bb_guest_view') === 'list';
  setProductsView(savedViewMode);

  if (viewToggleBtn) {
    viewToggleBtn.addEventListener('click', function() {
      const isCurrentlyList = productsGrid.classList.contains('bb-list-view');
      const nextIsList = !isCurrentlyList;
      setProductsView(nextIsList);
      sessionStorage.setItem('bb_guest_view', nextIsList ? 'list' : 'grid');
    });
  }
  ...
  function saveCart() {
    ...
    sessionStorage.setItem('bb_guest_cart_{{ $table->table_id }}', JSON.stringify(cart));
    renderCartSummary();
  }
      ...
      html += `
        <div class="pt-3 first:pt-0 flex items-center justify-between gap-3">
          <div class="flex-grow min-w-0">
            ...
            <div class="text-xs text-amber-800 font-bold mt-0.5">${formatRupiah(item.price * item.qty)} <span class="text-[11px] text-on-surface-variant font-normal">(${item.qty} x ${formatRupiah(item.price)})</span></div>
        ...
        <div class="pt-3 first:pt-0 flex items-center justify-between gap-3">
          <div class="flex-grow min-w-0">
            <div class="flex items-center gap-1">
              <span class="px-1.5 py-0.5 bg-amber-400 text-slate-900 text-[10px] font-extrabold rounded">PAKET</span>
              <h5 class="font-headline font-extrabold text-sm text-on-background truncate">${bName}</h5>
            </div>
            <div class="text-xs text-amber-800 font-bold mt-0.5">${formatRupiah(bPrice * b.qty)} <span class="text-[11px] text-on-surface-variant font-normal">(${b.qty} x ${formatRupiah(bPrice)})</span></div>
          </div>
  ...
  // Category Filtering
  const catPills = document.querySelectorAll('.bb-cat-pill');
  ...
  catPills.forEach(pill => {
    pill.addEventListener('click', function() {
      catPills.forEach(p => p.classList.remove('active', 'bg-amber-400', 'text-slate-900'));
  ...
  checkoutForm.addEventListener('submit', function(e) {
    ...
    document.getElementById('bbFormTotalPrice').value = grandTotal;
    document.getElementById('bbFormCartData').value = JSON.stringify(itemsPayload);
    document.getElementById('bbFormBundleData').value = JSON.stringify(bundlesPayload);

    // Disable button safely without breaking submit payload
    const checkoutBtn = document.getElementById('bbCheckoutBtn');
