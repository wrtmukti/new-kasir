...
<div class="guest-product-card bg-surface-container-low rounded-xl overflow-hidden shadow-xs hover:shadow-md border border-surface-variant bento-card flex flex-col justify-between"
  ...
  <div class="card-main-content">
    ...
    <div class="card-img-wrap relative h-48 w-full overflow-hidden bg-surface-variant">
      ...
      @else
        <div class="w-full h-full flex flex-col items-center justify-center text-on-surface-variant bg-surface-container-high">
          <span class="material-symbols-outlined text-4xl mb-1">bakery_dining</span>
    ...
    <div class="card-info-body p-4">
      <div class="flex items-center justify-between gap-1.5 mb-1">
        <span class="text-[10px] font-headline font-extrabold uppercase tracking-wider text-amber-800 opacity-80 truncate">
          {{ $product->category->category_name ?? 'Bumblebee Special' }}
        </span>
        <!-- Rating badge -->
        <div class="flex gap-0.5 items-center flex-shrink-0 bg-amber-100 px-1.5 py-0.5 rounded-full border border-amber-300">
          <span class="material-symbols-outlined text-amber-600 fill-icon text-[13px]">star</span>
          <span class="text-[10px] font-bold text-amber-900">4.9</span>
        </div>
      </div>
      ...
      <div class="card-price-row flex items-baseline justify-between gap-1.5 mt-auto">
        <div class="flex items-baseline gap-1.5 flex-wrap">
          <span class="font-headline font-black text-amber-800 text-base">
        ...
        @if($discountAmount > 0)
          <span class="px-1.5 py-0.5 bg-amber-500 text-slate-900 text-[9px] font-headline font-black uppercase rounded shadow-xs">
            DISKON
          </span>
        @endif
      </div>
  ...
  <div class="card-action-wrap p-4 pt-0">
    <button type="button" 
            class="btn-add-cart mt-2 w-full bg-amber-400 hover:bg-amber-500 text-slate-900 py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all font-headline font-bold text-xs active:scale-95 shadow-xs"
            data-id="{{ $product->product_id }}">
