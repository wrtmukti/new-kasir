...
<div class="max-w-3xl mx-auto">
  ...
  <div class="flex items-center gap-3 mb-6">
    <a href="{{ route('guest.index', $table->table_id) }}" class="p-2 text-amber-800 hover:bg-amber-100 rounded-full transition-colors" title="Kembali ke Menu">
      <span class="material-symbols-outlined">arrow_back</span>
    </a>
    <div>
      <h2 class="font-headline font-black text-2xl text-on-background">Konfirmasi Pesanan</h2>
      <p class="text-xs text-on-surface-variant font-bold">Meja {{ $table->table_number }} • {{ $company->company_name ?? 'Bumblebee Cafe' }}</p>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-xs mb-6">
    <h3 class="font-headline font-extrabold text-base text-on-background mb-4 flex items-center gap-2 border-b border-surface-variant pb-3">
      <span class="material-symbols-outlined text-amber-600 fill-icon">shopping_bag</span>
      Rincian Pesanan
    </h3>
    ...
    <div class="divide-y divide-surface-variant">
          ...
          <div class="flex items-center gap-3 flex-grow">
            ...
            <div>
              ...
              <div class="text-xs text-on-surface-variant">
                @if($item['qty'] > 1)<span class="font-bold text-amber-800">{{ $item['qty'] }}x</span> @endif
      ...
      @foreach($bundleRows ?? [] as $b)
        <div class="py-3.5 flex items-center justify-between gap-3">
          <div class="flex items-center gap-3 flex-grow">
            <div class="w-14 h-14 rounded-xl bg-amber-400 text-slate-900 flex items-center justify-center font-bold flex-shrink-0">
              <span class="material-symbols-outlined text-2xl fill-icon">emoji_nature</span>
            </div>
            <div>
              ...
              <div class="text-xs text-on-surface-variant mt-0.5">
                @if($b['qty'] > 1)<span class="font-bold text-amber-800">{{ $b['qty'] }}x</span> @endif
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-xs mb-6">
    <h3 class="font-headline font-extrabold text-base text-on-background mb-3 flex items-center gap-2">
      <span class="material-symbols-outlined text-amber-600">confirmation_number</span>
      Voucher Diskon
    </h3>
    
    <div class="flex gap-2">
      <input type="text" id="bbVoucherInput" value="{{ old('voucher_code') }}" placeholder="Masukkan kode voucher..."
             class="flex-grow bg-surface-container-low border border-surface-variant rounded-xl px-4 py-2.5 text-sm uppercase font-headline font-bold focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all"/>
      <button type="button" id="bbVoucherBtn" class="bg-amber-400 hover:bg-amber-500 text-slate-900 px-5 py-2.5 rounded-xl font-headline font-extrabold text-xs transition-all active:scale-95 shadow-xs">
        Gunakan
      </button>
    </div>
    <div id="bbVoucherResult" class="mt-2 text-xs"></div>
  </div>
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-xs mb-6">
    ...
    <textarea id="bbOrderRemark" rows="2" placeholder="Cth: less sugar, disajikan bersamaan..."
              class="w-full bg-surface-container-low border border-surface-variant rounded-xl p-3 text-sm focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all"></textarea>
  </div>
  ...
  <div class="bg-white rounded-2xl p-6 border border-surface-variant shadow-xs mb-6 space-y-3">
    ...
    <div id="bbVoucherRow" class="flex justify-between items-center text-sm font-headline hidden">
      <span class="text-amber-800 font-medium flex items-center gap-1">
        <span class="material-symbols-outlined text-[16px]">sell</span>
        Diskon Voucher
      </span>
      <span id="bbVoucherAmount" class="font-bold text-amber-800">-Rp 0</span>
    </div>
    ...
    <div class="border-t border-surface-variant pt-3 flex justify-between items-center text-lg font-headline">
      ...
      <span id="bbGrandTotal" class="font-black text-amber-800 text-xl">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
    </div>
  </div>

  <!-- Submit Form -->
  <form id="bbSubmitForm" method="POST" action="{{ route('guest.submit') }}">
    @csrf
    <input type="hidden" name="table_id" value="{{ $table->table_id }}"/>
    <input type="hidden" name="total_price" id="bbFormTotalPrice" value="{{ $grandTotal }}"/>
    <input type="hidden" name="items" id="bbFormItems"/>
    <input type="hidden" name="bundles" id="bbFormBundles"/>
    <input type="hidden" name="voucher_code" id="bbFormVoucherCode"/>
    <input type="hidden" name="order_remark" id="bbFormOrderRemark"/>

    <button type="submit" id="bbSubmitBtn" class="w-full bg-amber-400 hover:bg-amber-500 text-slate-900 py-4 rounded-xl font-headline font-black text-base flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const voucherInput = document.getElementById('bbVoucherInput');
  const voucherBtn = document.getElementById('bbVoucherBtn');
  const voucherResult = document.getElementById('bbVoucherResult');
  const voucherRow = document.getElementById('bbVoucherRow');
  const voucherAmountEl = document.getElementById('bbVoucherAmount');
  const grandTotalEl = document.getElementById('bbGrandTotal');

  const submitForm = document.getElementById('bbSubmitForm');
  const submitBtn = document.getElementById('bbSubmitBtn');
  ...
  function updateTotals() {
    ...
    document.getElementById('bbFormTotalPrice').value = finalTotal;
  ...
  voucherBtn.addEventListener('click', function() {
    ...
    .then(res => {
      ...
      if (res.ok) {
        ...
        appliedVoucher = null;
        voucherResult.innerHTML = `
          <div class="flex items-center gap-1 text-amber-800 font-headline font-bold bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200">
  ...
  submitForm.addEventListener('submit', function(e) {
    ...
    document.getElementById('bbFormItems').value = JSON.stringify(items);
    document.getElementById('bbFormBundles').value = JSON.stringify(bundles.map(b => ({
    ...
    document.getElementById('bbFormVoucherCode').value = appliedVoucher ? appliedVoucher.code : '';
    document.getElementById('bbFormOrderRemark').value = document.getElementById('bbOrderRemark').value.trim();

    // Clear saved local cart after submit
    sessionStorage.removeItem('bb_guest_cart_{{ $table->table_id }}');
