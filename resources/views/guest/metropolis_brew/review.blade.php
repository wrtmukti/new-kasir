...
<div class="max-w-3xl mx-auto">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      ...
      <p class="text-xs text-on-surface-variant font-bold">Meja {{ $table->table_number }} • {{ $company->company_name ?? 'Metropolis Brew' }}</p>
    </div>
  </div>
  ...
  <div class="bg-surface rounded-2xl p-6 border border-outline-variant/60 shadow-xs mb-6">
    ...
    <div class="divide-y divide-surface-variant">
      ...
      @foreach($items as $item)
        ...
        <div class="py-3.5 first:pt-0 flex items-center justify-between gap-3">
          <div class="flex items-center gap-3 flex-grow">
            @if($p->product_image)
              <img src="{{ asset('storage/' . $p->product_image) }}" alt="" class="w-14 h-14 rounded-xl object-cover border border-outline-variant/60 flex-shrink-0"/>
            @else
      ...
      @foreach($bundleRows ?? [] as $b)
        <div class="py-3.5 flex items-center justify-between gap-3">
          <div class="flex items-center gap-3 flex-grow">
            <div class="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center text-primary flex-shrink-0 border border-primary/20">
  ...
  <div class="bg-surface rounded-2xl p-6 border border-outline-variant/60 shadow-xs mb-6">
    ...
    <div class="flex gap-2">
      <input type="text" id="mbVoucherInput" value="{{ old('voucher_code') }}" placeholder="Masukkan kode voucher..."
             class="flex-grow bg-surface-container-low border border-outline-variant/70 rounded-xl px-4 py-2.5 text-sm uppercase font-headline font-bold focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"/>
      <button type="button" id="mbVoucherBtn" class="bg-primary hover:bg-primary-hover text-white px-5 py-2.5 rounded-xl font-headline font-extrabold text-xs transition-all active:scale-95 shadow-xs">
        Gunakan
      </button>
    </div>
    <div id="mbVoucherResult" class="mt-2 text-xs"></div>
  </div>
  ...
  <div class="bg-surface rounded-2xl p-6 border border-outline-variant/60 shadow-xs mb-6">
    ...
    <textarea id="mbOrderRemark" rows="2" placeholder="Cth: tolong dipisahkan gula, disajikan bersamaan..."
              class="w-full bg-surface-container-low border border-outline-variant/70 rounded-xl p-3 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"></textarea>
  </div>

  <!-- Price Summary Bento Card -->
  <div class="bg-surface rounded-2xl p-6 border border-outline-variant/60 shadow-xs mb-6 space-y-3">
    ...
    <div id="mbVoucherRow" class="flex justify-between items-center text-sm font-headline hidden">
      <span class="text-primary font-medium flex items-center gap-1">
        <span class="material-symbols-outlined text-[16px]">sell</span>
        Diskon Voucher
      </span>
      <span id="mbVoucherAmount" class="font-bold text-primary">-Rp 0</span>
    </div>
    ...
    <div class="border-t border-surface-variant pt-3 flex justify-between items-center text-lg font-headline">
      ...
      <span id="mbGrandTotal" class="font-black text-primary text-xl">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
    </div>
  </div>

  <!-- Submit Form -->
  <form id="mbSubmitForm" method="POST" action="{{ route('guest.submit') }}">
    @csrf
    <input type="hidden" name="table_id" value="{{ $table->table_id }}"/>
    <input type="hidden" name="total_price" id="mbFormTotalPrice" value="{{ $grandTotal }}"/>
    <input type="hidden" name="items" id="mbFormItems"/>
    <input type="hidden" name="bundles" id="mbFormBundles"/>
    <input type="hidden" name="voucher_code" id="mbFormVoucherCode"/>
    <input type="hidden" name="order_remark" id="mbFormOrderRemark"/>

    <button type="submit" id="mbSubmitBtn" class="w-full bg-primary hover:bg-primary-hover text-white py-4 rounded-xl font-headline font-black text-base flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const voucherInput = document.getElementById('mbVoucherInput');
  const voucherBtn = document.getElementById('mbVoucherBtn');
  const voucherResult = document.getElementById('mbVoucherResult');
  const voucherRow = document.getElementById('mbVoucherRow');
  const voucherAmountEl = document.getElementById('mbVoucherAmount');
  const grandTotalEl = document.getElementById('mbGrandTotal');

  const submitForm = document.getElementById('mbSubmitForm');
  ...
  function updateTotals() {
    ...
    document.getElementById('mbFormTotalPrice').value = finalTotal;
  ...
  submitForm.addEventListener('submit', function(e) {
    ...
    document.getElementById('mbFormItems').value = JSON.stringify(items);
    document.getElementById('mbFormBundles').value = JSON.stringify(bundles.map(b => ({
    ...
    document.getElementById('mbFormVoucherCode').value = appliedVoucher ? appliedVoucher.code : '';
    document.getElementById('mbFormOrderRemark').value = document.getElementById('mbOrderRemark').value.trim();

    // Clear saved local cart after submit
    sessionStorage.removeItem('mb_guest_cart_{{ $table->table_id }}');
