@extends('guest.metropolis_brew.layouts.app')
...
<section class="mb-8">
  <span class="text-xs font-headline font-extrabold uppercase tracking-widest text-primary bg-primary/10 px-3 py-1 rounded-full border border-primary/20">
    Urban Specialty Coffee
  </span>
  <h2 class="font-headline font-black text-3xl md:text-5xl text-on-background leading-tight mt-2">
    Crafted for the <br/><span class="text-primary">Urban Palate.</span>
  </h2>
  <p class="text-on-surface-variant text-sm md:text-base max-w-md mt-2">
    Kopi pilihan, bahan segar, & sajian perkotaan terbaik. Pilihlah menu di meja {{ $table->table_number }}.
  </p>
</section>
...
<section class="mb-6">
  <div class="relative w-full max-w-md">
    ...
    <input type="text" id="guestSearch" placeholder="Cari kopi, makanan, minuman..." 
           class="w-full bg-surface border border-outline-variant/70 rounded-full pl-11 pr-4 py-2.5 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"/>
  </div>
</section>
...
<section class="mb-8 -mx-4 px-4 overflow-hidden">
  <div class="flex gap-2.5 overflow-x-auto hide-scrollbar pb-2" id="guestCategoryContainer">
    <button class="mb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full bg-primary text-white font-headline font-bold text-xs transition-all active:scale-95 active" data-cat="all">
      Semua Menu
    </button>
    @if($bundles->isNotEmpty())
      <button class="mb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full border border-outline-variant text-on-surface-variant font-headline font-bold text-xs hover:bg-surface-container transition-all active:scale-95" data-cat="bundle">
        ☕ Paket Special
      </button>
    @endif
    @foreach($categories as $cat)
      <button class="mb-cat-pill flex-shrink-0 px-5 py-2.5 rounded-full border border-outline-variant text-on-surface-variant font-headline font-bold text-xs hover:bg-surface-container transition-all active:scale-95" data-cat="{{ $cat->category_id }}">
        {{ $cat->category_name }}
      </button>
    @endforeach
  </div>
...
@if($bundles->isNotEmpty())
  <div class="mb-8" id="guestBundlesSection">
    <h3 class="font-headline font-extrabold text-lg text-on-background mb-4 flex items-center gap-2">
      <span class="material-symbols-outlined text-primary fill-icon">local_cafe</span>
      Paket Hemat Urban
    </h3>
    <div class="bento-grid" id="guestBundles">
      @foreach($bundles as $bundle)
        @include('guest.metropolis_brew.partials._bundle_card', ['bundle' => $bundle])
      @endforeach
    </div>
  </div>
@endif
...
<section>
  <div class="flex items-center justify-between mb-4">
    ...
    <button type="button" id="mbViewToggle" class="p-2 rounded-full bg-surface border border-outline-variant/60 hover:bg-surface-container text-primary transition-colors flex items-center justify-center active:scale-95 shadow-xs" title="Tampilan Daftar">
      <span class="material-symbols-outlined text-[20px]" id="mbViewToggleIcon">format_list_bulleted</span>
    </button>
  </div>

  <div class="bento-grid" id="guestProducts">
    @foreach($products as $product)
      @include('guest.metropolis_brew.partials._product_card', ['product' => $product])
    @endforeach
  </div>

  @if($products->isEmpty() && $bundles->isEmpty())
    <div class="text-center py-16 bg-surface rounded-2xl border border-dashed border-outline-variant my-6">
...
<div id="mbCartBar" class="mb-cart-bar hidden cursor-pointer">
  <div class="flex items-center gap-3">
    <div class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center">
      <span class="material-symbols-outlined text-[20px] fill-icon">shopping_bag</span>
    </div>
    <div>
      <div id="mbCartCount" class="font-headline font-bold text-xs uppercase tracking-wide opacity-90">0 Item</div>
      <div id="mbCartTotal" class="font-headline font-black text-sm">Rp 0</div>
    </div>
  </div>
...
<div id="mbCartBackdrop" class="mb-cart-backdrop"></div>
<div id="mbCartSheet" class="mb-cart-sheet">
  <div class="w-12 h-1.5 bg-outline-variant/60 rounded-full mx-auto my-3"></div>
  <div class="px-6 py-3 border-b border-surface-variant flex items-center justify-between">
    ...
    <button type="button" id="mbCartClose" class="text-on-surface-variant hover:text-primary p-1 rounded-full">
      <span class="material-symbols-outlined">close</span>
    </button>
  </div>

  <div id="mbCartBody" class="p-6 overflow-y-auto max-h-[50vh] space-y-4 divide-y divide-surface-variant">
    <!-- Rendered via JS -->
  </div>
  ...
  <div class="p-6 border-t border-surface-variant bg-surface-container-low rounded-b-1.5rem">
    <div class="flex items-center justify-between mb-4">
      ...
      <span id="mbSheetTotal" class="font-headline font-black text-xl text-primary">Rp 0</span>
    </div>
    
    <form id="mbCheckoutForm" method="POST" action="{{ route('guest.checkout') }}">
      @csrf
      <input type="hidden" name="table_id" value="{{ $table->table_id }}"/>
      <input type="hidden" name="total_price" id="mbFormTotalPrice" value="0"/>
      <input type="hidden" name="cart_data" id="mbFormCartData" value="[]"/>
      <input type="hidden" name="bundle_data" id="mbFormBundleData" value="[]"/>
      
      <button type="submit" id="mbCheckoutBtn" class="w-full bg-primary hover:bg-primary-hover text-white py-3.5 rounded-xl font-headline font-extrabold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
...
<div id="mbItemModal" class="mb-modal-overlay">
  <div class="mb-modal-card">
    <div class="flex items-center justify-between border-b border-surface-variant pb-3 mb-4">
      <h4 id="mbModalTitle" class="font-headline font-extrabold text-base text-on-background truncate">Nama Menu</h4>
      <button type="button" id="mbModalClose" class="text-on-surface-variant hover:text-primary">
        <span class="material-symbols-outlined">close</span>
      </button>
    </div>
    ...
    <div class="mb-4">
      <div class="flex items-center justify-between mb-3">
        <span id="mbModalPrice" class="font-headline font-black text-lg text-primary">Rp 0</span>
        <div class="flex items-center gap-3 bg-surface-container-high rounded-full px-3 py-1">
          <button type="button" id="mbQtyMinus" class="w-8 h-8 rounded-full bg-white text-on-background flex items-center justify-center font-bold shadow-xs hover:bg-surface transition-all active:scale-90">
            <span class="material-symbols-outlined text-[18px]">remove</span>
          </button>
          <span id="mbQtyVal" class="font-headline font-bold text-sm min-w-[24px] text-center">1</span>
          <button type="button" id="mbQtyPlus" class="w-8 h-8 rounded-full bg-white text-on-background flex items-center justify-center font-bold shadow-xs hover:bg-surface transition-all active:scale-90">
      ...
      <input type="text" id="mbModalNote" placeholder="Cth: less sugar, oatmilk, ekstra shot..."
             class="w-full bg-surface-container-low border border-outline-variant/70 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"/>
    </div>

    <button type="button" id="mbAddToCartBtn" class="w-full bg-primary hover:bg-primary-hover text-white py-3 rounded-xl font-headline font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
      <span class="material-symbols-outlined text-[20px]">add_shopping_cart</span>
      <span id="mbAddToCartBtnText">Simpan Pesanan</span>
    </button>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // DOM Elements
  const cartBar = document.getElementById('mbCartBar');
  const cartCountEl = document.getElementById('mbCartCount');
  const cartTotalEl = document.getElementById('mbCartTotal');

  const cartSheet = document.getElementById('mbCartSheet');
  const cartBackdrop = document.getElementById('mbCartBackdrop');
  const cartClose = document.getElementById('mbCartClose');
  const cartBody = document.getElementById('mbCartBody');
  const sheetTotalEl = document.getElementById('mbSheetTotal');

  const modal = document.getElementById('mbItemModal');
  const modalTitle = document.getElementById('mbModalTitle');
  const modalPrice = document.getElementById('mbModalPrice');
  const modalNote = document.getElementById('mbModalNote');
  const modalClose = document.getElementById('mbModalClose');
  const qtyVal = document.getElementById('mbQtyVal');
  const qtyMinus = document.getElementById('mbQtyMinus');
  const qtyPlus = document.getElementById('mbQtyPlus');
  const addToCartBtn = document.getElementById('mbAddToCartBtn');
  const addToCartBtnText = document.getElementById('mbAddToCartBtnText');

  const checkoutForm = document.getElementById('mbCheckoutForm');
  ...
  const viewToggleBtn = document.getElementById('mbViewToggle');
  const viewToggleIcon = document.getElementById('mbViewToggleIcon');
  ...
  function setProductsView(isList) {
    ...
    if (isList) {
      productsGrid.classList.add('mb-list-view');
      viewToggleBtn.title = 'Tampilan Kartu';
      viewToggleIcon.textContent = 'grid_view';
    } else {
      productsGrid.classList.remove('mb-list-view');
      viewToggleBtn.title = 'Tampilan Daftar';
      viewToggleIcon.textContent = 'format_list_bulleted';
    }
  }

  const savedViewMode = sessionStorage.getItem('mb_guest_view') === 'list';
  setProductsView(savedViewMode);

  if (viewToggleBtn) {
    viewToggleBtn.addEventListener('click', function() {
      const isCurrentlyList = productsGrid.classList.contains('mb-list-view');
      const nextIsList = !isCurrentlyList;
      setProductsView(nextIsList);
      sessionStorage.setItem('mb_guest_view', nextIsList ? 'list' : 'grid');
    });
  }
  ...
  function saveCart() {
    ...
    sessionStorage.setItem('mb_guest_cart_{{ $table->table_id }}', JSON.stringify(cart));
    renderCartSummary();
  }
  ...
  // Category Filtering
  const catPills = document.querySelectorAll('.mb-cat-pill');
  ...
  checkoutForm.addEventListener('submit', function(e) {
    ...
    document.getElementById('mbFormTotalPrice').value = grandTotal;
    document.getElementById('mbFormCartData').value = JSON.stringify(itemsPayload);
    document.getElementById('mbFormBundleData').value = JSON.stringify(bundlesPayload);

    // Disable button safely without breaking submit payload
    const checkoutBtn = document.getElementById('mbCheckoutBtn');
