...
<div class="guest-bundle-card bg-surface-container-low rounded-xl overflow-hidden shadow-xs hover:shadow-md border border-outline-variant bento-card flex flex-col justify-between"
  ...
  <div>
    <div class="relative h-44 w-full overflow-hidden bg-primary/10 flex items-center justify-center">
      <div class="w-16 h-16 rounded-2xl bg-primary text-white flex items-center justify-center shadow-md">
        <span class="material-symbols-outlined text-3xl fill-icon">inventory_2</span>
      </div>
      <div class="absolute top-3 right-3 bg-primary text-white font-headline font-extrabold text-xs px-3 py-1 rounded-full shadow-xs">
        PAKET URBAN
      </div>
    </div>
  ...
  <div class="p-4 pt-0">
    <button type="button" 
            class="btn-add-bundle mt-2 w-full bg-primary-container hover:bg-primary text-white py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all font-headline font-extrabold text-xs active:scale-95 shadow-xs"
            data-bundle-id="{{ $bundle->bundle_id }}">
