...
<div class="guest-product-card bg-surface rounded-xl overflow-hidden shadow-xs hover:shadow-md border border-surface-variant bento-card flex flex-col justify-between"
  ...
  <div class="card-main-content">
    <!-- Product Image -->
    <div class="card-img-wrap relative h-48 w-full overflow-hidden bg-surface-container">
      ...
      @else
        <div class="w-full h-full flex flex-col items-center justify-center text-on-surface-variant bg-surface-container-high">
          <span class="material-symbols-outlined text-4xl mb-1">local_cafe</span>
    ...
    <div class="card-info-body p-4">
      <div class="flex items-center justify-between gap-1.5 mb-1">
        <span class="text-[10px] font-headline font-extrabold uppercase tracking-wider text-outline opacity-90 truncate">
          {{ $product->category->category_name ?? 'Signature Brew' }}
        </span>
        <!-- Brew rating badge -->
        <div class="flex gap-0.5 items-center flex-shrink-0 bg-surface-container-low px-1.5 py-0.5 rounded-full border border-outline-variant/50">
          <span class="material-symbols-outlined text-primary fill-icon text-[13px]">star</span>
          <span class="text-[10px] font-bold text-primary">4.9</span>
        </div>
      </div>
  ...
  <div class="card-action-wrap p-4 pt-0">
    <button type="button" 
            class="btn-add-cart mt-2 w-full bg-primary hover:bg-primary-container text-white py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all font-headline font-bold text-xs active:scale-95 shadow-xs"
            data-id="{{ $product->product_id }}">
