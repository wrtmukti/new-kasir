...
<div class="max-w-4xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-8">
    <div class="flex items-center gap-4">
      <div class="bg-primary p-3.5 rounded-2xl text-white shadow-md">
        <span class="material-symbols-outlined text-3xl fill-icon">local_cafe</span>
      </div>
      <div>
        <h2 class="font-headline font-black text-2xl md:text-3xl text-primary leading-tight">Urban Order Tracker</h2>
        <p class="text-xs md:text-sm text-on-surface-variant font-bold">Status Pesanan Meja {{ $table->table_number }}</p>
      </div>
    </div>

    <button type="button" id="mbRefreshBtn" class="bg-surface-container-high hover:bg-surface-variant text-on-surface p-2.5 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-xs" title="Refresh Status">
      <span class="material-symbols-outlined text-[20px]">refresh</span>
    </button>
  </div>

  @if($orders->isEmpty())
    <div class="text-center py-16 bg-surface rounded-2xl border border-dashed border-outline-variant shadow-xs my-6">
      ...
      <a href="{{ route('guest.index', $table->table_id) }}" class="inline-flex items-center gap-2 bg-primary hover:bg-primary-hover text-white px-6 py-3 rounded-xl font-headline font-bold text-xs shadow-md transition-all active:scale-95">
  ...
  @else
    <div class="space-y-6">
      @foreach($orders as $order)
        ...
        <div class="bg-surface rounded-2xl p-6 border border-outline-variant/60 shadow-xs bento-card">
          <!-- Order Card Header -->
          <div class="flex flex-wrap items-center justify-between gap-2 mb-4 border-b border-surface-variant pb-3">
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              ...
              @elseif($status === 'in_progress') bg-emerald-100 text-emerald-800
              @elseif($status === 'completed') bg-green-100 text-green-800
              @else bg-red-100 text-red-800 @endif">
              <span class="w-2 h-2 rounded-full @if($status === 'pending') bg-yellow-500 animate-ping @elseif($status === 'in_progress') bg-emerald-500 animate-pulse @elseif($status === 'completed') bg-green-500 @else bg-red-500 @endif"></span>
              {{ $statusLabel }}
            </span>
            </div>
          </div>
            ...
            <div class="my-6 px-2">
              <div class="grid grid-cols-3 gap-2 relative text-center">
                ...
                <div class="absolute top-5 left-1/6 right-1/6 h-1 bg-surface-container-high -z-0">
                  <div class="h-full bg-primary transition-all duration-500" style="width: {{ $stepDone ? '100%' : ($stepActive ? '50%' : '0%') }}"></div>
                </div>

                <!-- Step 1: Diterima -->
                <div class="flex flex-col items-center gap-1.5 z-10">
                  <div class="w-10 h-10 rounded-full bg-surface-container-high text-primary flex items-center justify-center font-bold shadow-sm border-2 border-white">
                ...
                <!-- Step 2: Dimasak -->
                <div class="flex flex-col items-center gap-1.5 z-10">
                  <div class="w-10 h-10 rounded-full {{ $stepActive ? 'bg-primary text-white' : 'bg-surface-container-high text-on-surface-variant' }} flex items-center justify-center font-bold shadow-sm border-2 border-white">
                ...
                <!-- Step 3: Selesai -->
                <div class="flex flex-col items-center gap-1.5 z-10">
                  <div class="w-10 h-10 rounded-full {{ $stepDone ? 'bg-green-600 text-white' : 'bg-surface-container-high text-on-surface-variant' }} flex items-center justify-center font-bold shadow-sm border-2 border-white">
            ...
            @foreach($order->products as $p)
              <div class="flex items-center justify-between text-sm">
                <div>
                  ...
                  @if($p->pivot->note)
                    <div class="text-xs text-on-surface-variant bg-surface px-2 py-0.5 rounded border border-outline-variant/50 mt-0.5">📝 {{ $p->pivot->note }}</div>
                  @endif
                </div>
              </div>
            @endforeach
    ...
    <div class="text-center mt-8">
      <a href="{{ route('guest.index', $table->table_id) }}" class="inline-flex items-center gap-2 bg-primary hover:bg-primary-hover text-white px-6 py-3 rounded-full font-headline font-bold text-xs shadow-md transition-all active:scale-95">
