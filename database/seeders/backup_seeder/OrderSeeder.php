    ...
    public function run(): void
    {
        ...
        for ($day = 1; $day <= $targetDays; $day++) {
            ...
            for ($i = 0; $i < $dailyOrderCount; $i++) {
                ...
                $closing = DailyClosing::where('outlet_id', $outlet->outlet_id)
                    ->where('business_date', $date->format('Y-m-d'))
                    ->where('shift_number', $shiftNumber)
                    ->first();
                $dailyClosingId = $closing ? $closing->id : null;

                DB::transaction(function () use ($outlet, $dailyClosingId, $orderType, $status, $grandTotal, $taxPercent, $taxAmount, $serviceChargePercent, $serviceChargeAmount, $tableId, $orderTimestamp, $syncData) {
