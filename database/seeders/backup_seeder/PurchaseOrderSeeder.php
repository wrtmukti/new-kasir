...
class PurchaseOrderSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $rawMaterials = RawStockMaterial::all()->keyBy('raw_material_code');
        ...
        // Helper: buat receiving + raw_stock_material_histories
        $createReceiving = function ($po, $items, $rcvSuffix, $date) use ($outlet) {
            ...
            $poItems = PurchaseOrderItem::where('po_id', $po->po_id)->get()->keyBy('raw_stock_material_id');

            foreach ($items as $item) {
                $poItem = $poItems[$item['raw']->raw_stock_material_id] ?? null;
                if (!$poItem) continue;
                ...
                PurchaseReceivingItem::create([
                    ...
                    'raw_stock_material_id' => $item['raw']->raw_stock_material_id,
                    'received_qty' => $qty,
        ...
        $savePo = function ($poCode, $supplierCode, $status, $items, $date, $notes = null, $cancelReason = null, $overrides = []) use ($outlet, $suppliers) {
            ...
            $paymentStatus = $overrides['payment_status'] ?? ($status === 'completed' ? 'paid' : 'unpaid');
            $paymentDate = $paymentStatus === 'paid' ? ($overrides['payment_date'] ?? $date) : null;
            $paymentMethod = $paymentStatus === 'paid' ? ($overrides['payment_method'] ?? 'transfer_bank') : null;
            $dueDate = $overrides['due_date'] ?? $date->copy()->addDays(14)->toDateString();
            ...
            foreach ($items as $item) {
                PurchaseOrderItem::create([
                    ...
                    'received_qty' => $overrides['received'][$item['raw']->raw_stock_material_id] ?? 0,
                ]);
            }
        ...
        $savePo('PO-001', 'SUP-001', 'draft', $items1, $baseDate, 'Draft PO bulanan bahan mentah', null, ['total' => $total1, 'payment_status' => 'unpaid']);

        // 2. PO-002 — COMPLETED (SUP-001: Beras, Cabai) -> PAID
        [$items2, $total2] = $makeItems(['RAW-BERAS01', 'RAW-CABE01'], [
            'RAW-BERAS01' => ['qty' => 30],
            'RAW-CABE01' => ['qty' => 5],
        ]);
        $po2 = $savePo('PO-002', 'SUP-001', 'completed', $items2, $baseDate->copy()->addDay(1), 'PO rutin mingguan beras & cabai', null, [
            'total' => $total2,
            'payment_status' => 'paid',
            'payment_method' => 'transfer_bank',
            'payment_date' => $baseDate->copy()->addDay(1)->addHours(2),
        ]);
        $createReceiving($po2, $items2, '20260722-001', $baseDate->copy()->addDay(1)->addHours(4));

        // 3. PO-003 — ORDERED (SUP-002: Minyak) -> UNPAID TEMPO
        [$items3, $total3] = $makeItems(['RAW-MINYAK01'], ['RAW-MINYAK01' => ['qty' => 20]]);
        $savePo('PO-003', 'SUP-002', 'ordered', $items3, $baseDate->copy()->addDays(2), 'PO minyak goreng', null, [
            'total' => $total3,
            'payment_status' => 'unpaid',
            'due_date' => $baseDate->copy()->addDays(16)->toDateString(),
        ]);
        ...
        $savePo('PO-004', 'SUP-002', 'cancelled', $items4, $baseDate->copy()->addDays(3),
            'PO minyak tambahan', 'Stok minyak masih mencukupi, PO tidak jadi dilanjutkan', ['total' => $total4, 'payment_status' => 'unpaid']);

        // 5. PO-005 — PARTIAL (SUP-003: Ayam) -> UNPAID TEMPO
        ...
        $po5 = $savePo('PO-005', 'SUP-003', 'partial', $items5, $baseDate->copy()->addDays(4),
            'PO daging ayam segar', null, [
                'total' => $total5,
                'payment_status' => 'unpaid',
                'due_date' => $baseDate->copy()->addDays(18)->toDateString(),
                'received' => [$rawMaterials['RAW-AYAM01']->raw_stock_material_id => 15],
            ]);
        ...
        RawStockMaterialHistory::create([
            'outlet_id' => $outlet->outlet_id,
            'raw_stock_material_id' => $rawAyam->raw_stock_material_id,
            'name' => $rawAyam->name,
            'unit' => $rawAyam->unit,
        ...
        $savePo('PO-007', 'SUP-004', 'draft', $items7, $baseDate->copy()->addDays(7), 'PO bumbu dapur', null, ['total' => $total7, 'payment_status' => 'unpaid']);
        ...
        $savePo('PO-008', 'SUP-004', 'cancelled', $items8, $baseDate->copy()->addDays(8),
            'PO cabai tambahan', 'Harga cabai turun, PO ditunda', ['total' => $total8, 'payment_status' => 'unpaid']);

        // 9. PO-009 — PARTIAL (SUP-005: Plastik & Gelas) -> PAID TRANSFER
        ...
        $po9 = $savePo('PO-009', 'SUP-005', 'partial', $items9, $baseDate->copy()->addDays(10),
            'PO kemasan', null, [
                ...
                'payment_status' => 'paid',
                'payment_method' => 'transfer_bank',
                'payment_date' => $baseDate->copy()->addDays(10),
                'received' => [
                    $rawMaterials['RAW-PLST01']->raw_stock_material_id => 100,
                    $rawMaterials['RAW-GLN01']->raw_stock_material_id => 200,
                ],
            ]);
        $createReceiving($po9, [
        ...
        // 10. PO-010 — ORDERED (SUP-005: Plastik & Gelas) -> UNPAID TEMPO
        ...
        $savePo('PO-010', 'SUP-005', 'ordered', $items10, $baseDate->copy()->addDays(12),
            'PO kemasan bulan depan', null, [
                'total' => $total10,
                'payment_status' => 'unpaid',
                'due_date' => $baseDate->copy()->addDays(26)->toDateString(),
            ]);

        $this->command->info('✅ 10 PO Bahan Mentah berhasil di-seed dengan status pembayaran lengkap.');
    }
}
